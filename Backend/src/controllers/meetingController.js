...
export const createMeeting = catchAsync(async (req, res, next) => {
  const { sellRequestId, type, scheduledDate, durationMinutes, location, officialId } = req.body;
  ...
  await Notification.create({
    ...
    message: `A ${type} meeting has been scheduled for your ${sellRequest.brand} ${sellRequest.model} on ${new Date(scheduledDate).toLocaleString()}.`,
    type: 'System'
  });

  const populatedMeeting = await Meeting.findById(meeting._id)
    .populate('sellRequest', 'brand model variant year city')
...
export const getAllMeetings = catchAsync(async (req, res, next) => {
  const { page = 1, limit = 20, status, type, official, dateFrom, dateTo } = req.query;
...
export const getMeetingByIdAdmin = catchAsync(async (req, res, next) => {
...
export const updateMeeting = catchAsync(async (req, res, next) => {
  const allowedUpdates = [
    'type', 'scheduledDate', 'durationMinutes', 'location', 'official',
    'officialNotes'
  ];
  ...
  if (updates.scheduledDate) {
    const sellRequest = await SellRequest.findById(meeting.sellRequest._id).populate('client');
    if (sellRequest) {
      await Notification.create({
        recipient: sellRequest.client._id,
        title: 'Meeting Rescheduled',
        message: `Your ${meeting.type} meeting has been rescheduled to ${new Date(updates.scheduledDate).toLocaleString()}.`,
        type: 'System'
      });
    }
  }
...
export const updateMeetingStatus = catchAsync(async (req, res, next) => {
  ...
  if (validTransitions[meeting.status] && !validTransitions[meeting.status].includes(status)) {
    return next(new AppError(`Cannot transition from ${meeting.status} to ${status}`, 400));
  }

  meeting.status = status;
  if (officialNotes) meeting.officialNotes = officialNotes;
  if (status === 'completed') meeting.completedAt = new Date();
  await meeting.save();

  const sellRequest = await SellRequest.findById(meeting.sellRequest._id).populate('client');

  if (status === 'completed' && sellRequest) {
    await Notification.create({
      recipient: sellRequest.client._id,
      title: 'Meeting Completed',
      message: `Your ${meeting.type} meeting for ${sellRequest.brand} ${sellRequest.model} has been completed.`,
      type: 'System'
    });
  }
